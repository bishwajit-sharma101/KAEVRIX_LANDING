...
import { MOCK_APP_ROADMAP, MOCK_USER_PROFILE } from "../../utils/mockInterceptor";
...
export default function WelcomeScreen({
  ...
  const [authMode, setAuthMode] = useState("menu"); // landing, menu, signin, signup
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      if (typeof window !== "undefined") {
        localStorage.setItem("kaevrix_roadmap_progress_TuringScholar", JSON.stringify(MOCK_APP_ROADMAP));
        localStorage.setItem("kaevrix_roadmap_TuringScholar", JSON.stringify(MOCK_APP_ROADMAP));
      }
      ...
      const res = await fetch(`${BACKEND_URL}/api/auth/login`, {
        ...
        credentials: "include",
        body: JSON.stringify({ username: loginUsername || "TuringScholar", password: loginPassword || "KaevrixDemoToken2026" })
      });
      ...
      onAuthSuccess(data.user || MOCK_USER_PROFILE, data.token || "kaevrix_demo_token_2026");
    } catch (err) {
          ...
          <div style={{
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                ...
                {[
                  ...
                  <button
                    ...
                    onMouseEnter={() => { if (!item.disabled) { sound.playClockTick(true); setHoveredButtonId(item.id); } }}
                    ...
                    style={{
                      ...
                      background: item.disabled
                        ...
                        : (hoveredButtonId === item.id
                          ...
                          : "transparent"),
                      border: `2px solid ${item.disabled ? (isDarkMode ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.05)") : (hoveredButtonId === item.id ? currentThemeColor : (isDarkMode ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.08)"))}`,
                      borderRadius: "16px",
                      padding: "18px 24px",
                      cursor: item.disabled ? "not-allowed" : "pointer",
                      opacity: item.disabled ? 0.35 : 1,
                      filter: item.disabled ? "grayscale(80%)" : "none",
                      display: "flex",
                      alignItems: "center",
                      gap: "18px",
                      transition: "all 0.2s ease",
                      transform: (!item.disabled && hoveredButtonId === item.id) ? "translateX(8px)" : "translateX(0)",
                      boxShadow: (!item.disabled && hoveredButtonId === item.id) ? `0 0 20px ${currentThemeColor}22` : "none",
                    }}
                  >
                    <span style={{
            ...
            {authMode === "signin" && (
              <div style={{ display: "flex", flexDirection: "column", gap: "0" }}>
                <button
                  onClick={() => { sound.playClockTick(); setAuthMode("menu"); setLoginError(""); }}
                ...
                <div style={{ marginBottom: "28px" }}>
                  ...
                  <div style={{ fontSize: "12px", color: textMuted, marginTop: "4px", letterSpacing: "1px" }}>
                    Demo credentials pre-loaded. Click authenticate to begin.
                  </div>
                </div>
                  ...
                  {!retroShowForm && (
                    <div style={{ display: "flex", flexDirection: "column", gap: "18px", justifyContent: "center" }}>
                      ...
                      {[
                        ...
                        { id: "newgame", label: "START NEW CAMPAIGN [ SIGN UP ] (LOCKED)", desc: "Restricted in Demo Mode", disabled: true, action: () => {} },
                        ...
                        <button
                          ...
                          style={{
                            ...
                            border: `2px solid ${item.disabled ? (isDarkMode ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.08)") : (hoveredButtonId === item.id ? currentThemeColor : (isDarkMode ? `${currentThemeColor}22` : `${currentThemeColor}44`))}`,
                            borderRadius: "12px",
                            opacity: item.disabled ? 0.35 : 1,
                            cursor: item.disabled ? "not-allowed" : "pointer",
                            filter: item.disabled ? "grayscale(80%)" : "none",
                            color: item.disabled ? textMuted : (hoveredButtonId === item.id ? (isDarkMode ? "#fff" : currentThemeColor) : textColor),
                            fontFamily: "var(--font-gamer)", fontSize: "18px", fontWeight: "900",
                            letterSpacing: "2.5px", display: "flex", flexDirection: "column",
                            gap: "6px", padding: "18px 24px", transition: "all 0.2s ease-out",
                            boxShadow: (!item.disabled && hoveredButtonId === item.id) ? `0 0 20px ${currentThemeColor}33` : "none",
                            transform: (!item.disabled && hoveredButtonId === item.id) ? "scale(1.02)" : "scale(1)"
                          }}
                        >
                  ...
                  {retroShowForm && authMode === "signin" && (
                    <form onSubmit={handleLoginSubmit} style={{ display: "flex", flexDirection: "column", gap: "18px", justifyContent: "center" }}>
                      ...
                      <div style={{ fontSize: "11px", color: textMuted, fontWeight: "900", letterSpacing: "3px", textTransform: "uppercase", marginBottom: "5px" }}>
                        ▶ Credentials Validation [DEMO ACCOUNT]
                      </div>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                        <span style={{ fontSize: "11px", color: currentThemeColor, fontFamily: "var(--font-gamer)", fontWeight: "bold", letterSpacing: "1.5px" }}>GAMER_TAG (PREFILLED):</span>
                        <input
                          type="text"
                          value={loginUsername}
                          readOnly={true}
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                        <span style={{ fontSize: "11px", color: currentThemeColor, fontFamily: "var(--font-gamer)", fontWeight: "bold", letterSpacing: "1.5px" }}>PASSKEY_HASH (PREFILLED):</span>
                        <input
                          type="password"
                          value={loginPassword}
                          readOnly={true}
