...
export default function GameArena({
  ...
  useEffect(() => {
    ...
    const handleResult = ({ questionIdx, isCorrect, correctAnswerIdx: serverCorrectIdx, submittedAnswerIdx, xpGained, scoreGained }) => {
      if (isCorrect) {
        sound.playCorrect();
        setResultMessage(`[COMPLETE] Correct! +${xpGained} XP`);
      } else {
        sound.playIncorrect();
        setResultMessage(submittedAnswerIdx === -1 ? `⏰ Time's Up!` : `[X] Incorrect!`);
      }
      ...
      <main className="arena-video-pane">
        ...
        <div className="video-player-outer-wrapper">
          ...
          {isFrozen && (
            <div className="debuff-overlay-freeze">
              <div className="debuff-text-freeze">[XP] EMP FREEZE ACTIVE [XP]</div>
          ...
          {isBlurred && (
            <div className="debuff-overlay-blur">
              <div className="debuff-text-blur">[FOG]️ SMOKE SCREEN ACTIVE [FOG]️</div>
        ...
        <div className="video-title-bar">
          <div>
            ...
            <p className="arena-video-creator">[VIDEO] {room?.video.channel} | Category: {room?.video.category}</p>
          </div>
          <button 
      ...
      <aside className="dashboard-sidebar">
        ...
        <div className="energy-bar-container glowing-border-cyan">
          <div className="energy-bar-header">
            <span>[XP] SYSTEM BATTERY POWERS</span>
            <span>{energy}%</span>
          </div>
